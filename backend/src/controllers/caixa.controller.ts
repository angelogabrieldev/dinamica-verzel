...
*/
export const finalizarCaixa = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (!caixa) res.status(404).json({ message: "Caixa não encontrado" });
    if (caixa!.status === StatusCaixa.FECHADO) res.status(400).json({ message: "Caixa já está fechado" });
